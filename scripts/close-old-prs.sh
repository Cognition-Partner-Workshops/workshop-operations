...
log "Cutoff date (PRs created before this will be closed): $CUTOFF_DATE"
log "Threshold: $OLDER_THAN_WEEKS weeks"
log "Dry run: $DRY_RUN"
log ""
  ...
  while IFS=$'\t' read -r PR_NUM CREATED_AT PR_TITLE; do
    if [[ "$CREATED_AT" < "$CUTOFF_DATE" ]]; then
      if [[ "$DRY_RUN" == true ]]; then
        ...
        CLOSE_COMMENT="Closing: this PR is older than $OLDER_THAN_WEEKS weeks. Reopen if still needed."
        if gh pr close "$PR_NUM" --repo "$FULL_REPO" --comment "$CLOSE_COMMENT" >/dev/null 2>&1; then
          log "  Closed PR #$PR_NUM: $PR_TITLE (created: $CREATED_AT)"
