...
#   --include=<pattern> Only deploy to repos matching this glob pattern
#   --exclude=<pattern> Skip repos matching this glob pattern
...
#
# This creates a PR in each target repo adding the PII check workflow.
# After merging, set up branch protection rules to require the check to pass.
...
for repo_name in "${REPOS[@]}"; do
  ...
  # Clone, create branch, add workflow, push, create PR
...
After merging, enable branch protection on \`$default_branch\` and require the **PR PII Check** status check to pass before merging.

See [operator repo](https://github.com/$ORG/operator) for details." 2>>"$LOGFILE" || echo "")
