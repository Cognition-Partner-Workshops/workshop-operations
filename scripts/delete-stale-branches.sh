#!/usr/bin/env bash
# delete-stale-branches.sh — Delete branches with no commits in N weeks (excluding default branch)
#
# Usage:
...
log "Cutoff date (branches with last commit before this are stale): $CUTOFF_DATE"
log "Stale threshold: $STALE_WEEKS weeks"
log "Dry run: $DRY_RUN"
log ""
...
for REPO_NAME in $REPOS; do
  ...
  # Using gh api to get branches with commit info
  PAGE=1
  while true; do
    ...
    while IFS=$'\t' read -r BRANCH_NAME COMMIT_SHA; do
      # Skip default branch and protected patterns
      ...
      if [[ -z "$COMMIT_DATE" ]]; then
        log "  SKIP $BRANCH_NAME (could not get commit date)"
        continue
      fi
      ...
      if [[ "$COMMIT_DATE" < "$CUTOFF_DATE" ]]; then
        if [[ "$DRY_RUN" == true ]]; then
          log "  [DRY-RUN] Would delete branch: $BRANCH_NAME (last commit: $COMMIT_DATE)"
        else
          if gh api "repos/$FULL_REPO/git/refs/heads/$BRANCH_NAME" -X DELETE >/dev/null 2>&1; then
            log "  Deleted branch: $BRANCH_NAME (last commit: $COMMIT_DATE)"
            TOTAL_DELETED=$((TOTAL_DELETED + 1))
          else
            log "  ERROR: Failed to delete branch: $BRANCH_NAME"
          fi
        fi
      fi
    done <<< "$BRANCHES"
...
log "Branches deleted: $TOTAL_DELETED"
